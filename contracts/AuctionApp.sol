...
contract Auction{
   ...
   // uint public immutable endTime;
    ...
    function bid() external onlyBefore(creationTime +3 minutes) payable{
        ...
        if(!(msg.value>highestBid)) revert("Amount not higher than current bid");
    ...
    function endAuction() external onlyBy payable{

        if(block.timestamp<creationTime + 3 minutes) revert("Auction has notended");
